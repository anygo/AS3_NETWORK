...
namespace as2DotNet
{
    ...
    public class ParamDecoder
    {
	     ...
	     * @throws Exception
	     */
        public DataVO decoder(byte[] data, IParamResolve outDecoder)
        {
            ...
            if (reader.BaseStream.Length < reader.ReadInt32())
                throw new Exception("丢包了");

            dataVo.ACUID = readString(reader); // 读取通信ID
        ...
        public Object readAll(BinaryReader reader)
        {
		    ...
		    {
                outer = _outDecoder.decode(type, reader);
		    }
...
//                doc.LoadXml(readString(reader));
		    ...
		    {
                long ts = DateTime.Parse("1970-1-1").Ticks + (long)reader.ReadDouble();
                param = new DateTime(ts);//DateTime.ParseExact(readString(reader), "yyyy-MM-dd HH:mm:ss:fff", null);
		    }
        ...
        public Object readModel(BinaryReader reader)
        {
            Object model = null;
            string nameSpace = readString(reader);

            Type modelCls = ContextUtil.getInstance(null).getModel(nameSpace);
            model = System.Activator.CreateInstance(modelCls);
            int len = reader.ReadInt32();

            string fieldName;
            
            PropertyInfo[] ps = modelCls.GetProperties();
            Hashtable hb = new Hashtable();
            foreach (PropertyInfo property in ps)
            {
                hb.Add(property.Name, property);
            }
            ...
            for (int loop = 0; loop < len; loop++)
            {
                ...
                if (hb.ContainsKey(fieldName))
                {
                    prop = (PropertyInfo) hb[fieldName];
                    prop.SetValue(model, param, null);
                }                
            }
